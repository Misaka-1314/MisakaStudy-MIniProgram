...
// @grant        GM_listValues
// @updateURL    https://raw.githubusercontent.com/Misaka-1314/MisakaStudy-MIniProgram/refs/heads/main/resource/油猴脚本/一键提审.js
// @downloadURL  https://raw.githubusercontent.com/Misaka-1314/MisakaStudy-MIniProgram/refs/heads/main/resource/油猴脚本/一键提审.js
// ==/UserScript==
...
(() => {
    ...
    button.addEventListener("click", async () => {
        ...
        document.getElementById("one-click-submit").style.backgroundColor =
            "#006CBE";
        setInterval(
            () =>
        ...
        fetch(
            ...
            )}&token=${token}&lang=${lang}&random=${random}`,
            {
                method: "GET",
                headers: {
            ...
            .then(() =>
                Promise.all([
                    fetch(
                        `https://mp.weixin.qq.com/wxamp/cgi/route?path=${encodeURIComponent(
                            "/wxopen/wacodepage?action=check_domain"
                        )}&token=${token}&lang=${lang}&random=${random}`,
                        {
                            method: "POST",
                            headers: {
                    ...
                    fetch(
                        `https://mp.weixin.qq.com/wxamp/cgi/route?path=${encodeURIComponent(
                            "/wxopen/waexperiencecode?action=check_plugin_version"
                        )}&token=${token}&lang=${lang}&random=${random}`,
                        {
                            method: "POST",
                            headers: {
            ...
            .then((class_info) =>
                fetch(
                    `https://mp.weixin.qq.com/wxamp/cgi/route?path=${encodeURIComponent(
                        "/wxopen/wadevelopcode?action=submit_check"
                    )}&token=${token}&lang=${lang}&random=${random}`,
                    {
                        method: "POST",
                        headers: {
                        ...
                        body: toFormData({
                            ...
                            encrypted_username: "",
                            encrypted_password: "",
                            remark: class_info.remark || getRemark(),
                            order_path: "",
                            feedback_info: "",
                            feedback_status: 1,
                            only_run_wxwork: 0,
